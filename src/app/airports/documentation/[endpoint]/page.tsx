...
function getFetchUrl(host: string, endpoint: string) {
  ...
  const isNonCollectionEndpoint = ["locations", "random-airport"].includes(
    endpoint,
  );
  const path = isAirportsHost
    ? `/api/${endpoint}${isNonCollectionEndpoint ? "" : "?limit=25"}`
    : `/api/airports/${endpoint}${isNonCollectionEndpoint ? "" : "?limit=25"}`;
...
export default async function Documentation({
  ...
  const isRandomAirportEndpoint = endpoint === "random-airport";
  const isCollectionEndpoint = !isLocationsEndpoint && !isRandomAirportEndpoint;
  ...
  return (
    <DocumentationPage
      ...
      showValuesView={isCollectionEndpoint && Array.isArray(valuesData)}
